...
data <- read_tsv("data/derived_data/data.tsv") %>%
  filter(variable == "agb", site != "Kibale") %>%
  unique() %>%
  rename(stem = value) %>%
...
unlink("data/derived_data/agb/stp_norm", recursive = TRUE)
dir.create("data/derived_data/agb/stp_norm")
fit_stp_norm <- stp_norm$sample(
  data = mdata,
  chains = 4,
  parallel_chains = 4,
  save_warmup = FALSE,
  output_dir = "data/derived_data/agb/stp_norm"
)
```
...
#| message: false
#| warning: false
fit_stp_norm <- as_cmdstan_fit(list.files("data/derived_data/agb/stp_norm",
  full.names = TRUE
))
fit_stp_norm
```
...
bind_rows(data_pre, data_rec, mutate(data_old, rel_year = Inf)) %>%
