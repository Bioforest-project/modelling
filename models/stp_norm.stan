...
parameters {
  real<lower=0, upper=8> mu_theta0 ; // starting point
  real<lower=0> sigma_theta0 ;
  vector<lower=0, upper=8>[n_plot_rec] theta0_p ;
  real<lower=0, upper=0.5> mu_lambda ; // recovery rate
  real<lower=0> sigma_lambda ;
  vector<lower=0, upper=0.5>[n_plot_rec] lambda_p ;
  real<lower=0, upper=8> mu_thetaInf ; // ending point
  real<lower=0> sigma_thetaInf ;
  vector<lower=0, upper=8>[n_site] thetaInf_s ;
...
model {
  log(stem_old) ~ normal(mu_old, sigma_old) ;
  log(stem_pre) ~ normal(mu_pre, sigma_pre) ;
  log(stem_rec) ~ normal(mu_rec, sigma_rec) ;
  theta0_p ~ normal(mu_theta0, sigma_theta0) ;
