...
#| message: false
#| warning: false
fit_stp_norm_false <- as_cmdstan_fit(list.files("data/derived_data/stp_norm_false",
  full.names = TRUE
))
fit_stp_norm_false
```
...
#| message: false
#| warning: false
fit_stp_norm_false2 <- as_cmdstan_fit(list.files("data/derived_data/stp_norm_false2",
  full.names = TRUE
))
fit_stp_norm_false2
```
...
bind_rows(
  fit_stp_norm$draws(pars) %>%
    as_tibble() %>%
    gather() %>%
    separate(key, c("chain", "parameter", "index", "ind_value"), convert = T) %>%
...
bind_rows(
  fit_stp_norm$draws(pars) %>%
    as_tibble() %>%
    gather() %>%
    separate(key, c("chain", "parameter"), sep = "\\.", convert = T) %>%
    mutate(model = "STP normal"),
  fit_stp_norm_false$draws(pars) %>%
    as_tibble() %>%
    gather() %>%
