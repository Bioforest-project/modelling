...
for(var in vars){
  ...
  pars_a <- fit$summary(c("lp__", "mu_dist", "mu_lambda", "mu_thetaInf",
                          "mu_delta", "mu_tau")) %>% 
    rename(parameter = variable) %>% 
    mutate(level = "inter", site = NA, plot = NA, 
           sitenum = NA, plotnum_rec = NA) %>% 
    select(parameter, level, site, plot, sitenum, plotnum_rec,
  ...
  pars_s <- fit$summary(c("tau_s", "thetaInf_s")) %>% 
    ...
    select(parameter, level, site, plot, sitenum, plotnum_rec,
           mean, median, sd, mad, q5, q95, 
           rhat, ess_bulk, ess_tail)
  pars_p <- fit$summary(c("dist_p", "delta_p", "lambda_p", "t95_p")) %>% 
...
biotic <- read_tsv("data/derived_data/data.tsv") %>% 
  ...
  filter(variable %in% c("gini", "SLA_cwm_ba", "WD_cwm_ba",
...
read_tsv(list.files(tmp_path, pattern = "pars", full.names = TRUE)) %>% 
  ...
  mutate(continent = ifelse(site %in% c("Kibale", "Mbaiki"), "Africa", continent)) %>% 
  mutate(continent = ifelse(site %in% c("Sg Lalang", "Ulu Muda",
                                        "Uppangala", "Malinau", "Lesong"),
                            "Asia", continent)) %>% 
  mutate(type = "structure") %>% 
  mutate(type = ifelse(attribute %in% c("diversity_q1_gen", "FDiv"), 
                       "diversity", type)) %>% 
  mutate(type = ifelse(attribute %in% c("SLA_cwm_ba", "WD_cwm_ba",
                                        "hmax_cwm_ba"), "functional", type)) %>% 
...
-   **Raw and derived parameters**: model parameters for each attribute at **inter-site**, **site** & **plot** levels (see <https://bioforest-project.github.io/modelling/21_models.html#stp>)
    ...
    -   **t95**: recovery time $t_{95}$, derived from $\lambda$ with $t_{95}=\frac{log(20)}{\lambda}$
-   **Parameter metrics**
...
-   **Variables**: associated variables at site or plot level (see <https://bioforest-project.github.io/environment/>)
    ...
    -   biotic: mean site pre-logging and control gini index **gini_s**, specific leaf area **SLA_cwm_ba_s**, wood density **WD_cwm_ba_s**, genus diversity **diversity_q1_gen_s**, and functional diversity **FDiv_s**
...
read_tsv("outputs/parameters.tsv") %>% 
  ...
  theme_bw() +
  coord_flip() +
  ylab(expression(theta[Inf])) + xlab("")
```
...
#| message: false
#| warning: false
#| fig-cap: "dist across sites & plots for AGB"
read_tsv("outputs/parameters.tsv") %>% 
  ...
  theme_bw() +
  coord_flip() +
  ylab(expression(dist)) + xlab("Plot") +
  ggtitle("AGB")
```
...
read_tsv("outputs/parameters.tsv") %>% 
  ...
  theme_bw() +
  coord_flip() +
  ylab(expression(delta)) + xlab("Plot") +
...
#| message: false
#| warning: false
#| fig-cap: "t95 across sites & plots for AGB"
read_tsv("outputs/parameters.tsv") %>% 
...
mu_lambda across attributes

mu_T95 accros attributes

> Q2. How does recovery time vary with disturbance intensities?

lambda_p versus Dist_p_ba across attributes (site in color)

T95_p versus Dist_p_ba across attributes (site in color)
