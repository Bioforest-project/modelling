...
data_rec <- read_tsv("data/derived_data/data_rec.tsv", col_types = cols()) %>%
  ...
  select(-sitenum) %>% 
  left_join(index)
model_path <- file.path("models", paste0("stp2", ".stan"))
stan_model <- cmdstan_model(model_path)
ind_rec <- data_rec %>%
...
fit2 <- stan_model$sample(
    data = list(
      ...
      n_plot = max(data_rec$plotnum_rec),
      y = data_rec$y,
      ...
      time_max = 60,
      thetaInf_bounds = c(1, 100),
      mu_thetaInf_s = fit$summary("mu")$median,
      sigma_thetaInf_s = fit$summary("mu")$sd,
      lambda_bounds = c(0,0.5),
