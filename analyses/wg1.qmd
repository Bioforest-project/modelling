...
data.frame(time = seq(0.1, 50, length.out = 100)) %>%
  ...
  geom_segment(aes(x = 6, y = 9, xend = 6, yend = 7),
    arrow = arrow(length = unit(0.2, "cm")), col = "#a6d7eb"
  ) +
  theme(axis.title = element_blank()) +
...
projs %>%
  ...
  geom_boxplot(
    data = data_old %>% mutate(rel_year = 50),
    width = 4, fill = NA
  ) +
  geom_jitter(
    data = data_old %>% mutate(rel_year = 50),
  ...
  geom_boxplot(
    data = data_pre %>% mutate(rel_year = 0),
    width = 4, fill = NA
  ) +
  geom_point(
    aes(col = plot),
...
dist <- read_tsv("../outputs/parameters.tsv") %>%
  filter(attribute == "ba", parameter == "dist_pct", site == params$site)
...
delta <- read_tsv("../outputs/parameters.tsv") %>%
  filter(
    ...
    parameter == "delta_pct", site == params$site
  )
